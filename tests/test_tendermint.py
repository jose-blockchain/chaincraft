...
class TestTendermintBFT(unittest.TestCase):
    ...
    def test_create_tendermint_network_returns_objects(self):
        ...
        try:
            ...
            num_nodes = 2
            nodes, tendermint_objs, tendermint_nodes = create_tendermint_network(num_nodes=num_nodes, consensus_interval=0.5)
            ...
            self.assertTrue(all(isinstance(obj, TendermintBFT) for obj in tendermint_objs))
            self.assertTrue(all(isinstance(node, TendermintNode) for node in tendermint_nodes))
        finally:
