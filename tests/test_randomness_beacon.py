...
def create_beacon_network(num_nodes, difficulty_bits=8):
    ...
    for _ in range(num_nodes):
        ...
        beacon = RandomnessBeacon(coinbase_address=coinbase, difficulty_bits=difficulty_bits)
...
def wait_for_chain_sync(beacons, expected_height, timeout=30):
    ...
    while time.time() - start_time < timeout:
        ...
        if all(h >= expected_height for h in heights):
            ...
            top_hashes = [beacon.blocks[expected_height - 1]["blockHash"] for beacon in beacons]
...
class TestRandomnessBeacon(unittest.TestCase):
    ...
    def test_create_beacon(self):
        ...
        self.assertEqual(beacon.blocks[0]["coinbaseAddress"], "0x0000000000000000000000000000000000000000")
    
    def test_mine_block(self):
        """Test mining a block"""
        # Create beacon with low difficulty for fast test
        coinbase = generate_eth_address()
        beacon = RandomnessBeacon(coinbase_address=coinbase, difficulty_bits=self.__DIFFICULTY)
        ...
        self.assertTrue(beacon.pow_primitive.verify_proof(challenge, block["nonce"], block["blockHash"]))
    ...
    def test_invalid_blocks(self):
        ...
        bad_prev["prevBlockHash"] = "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
    ...
    def test_block_collision(self):
        ...
        for nonce in range(1000):
            ...
            if better_block["blockHash"] < block1["blockHash"]:
                found_better = True
                print(f"Found better hash with nonce {nonce}: {better_block['blockHash']}")
                break
        ...
        self.assertEqual(beacon.ledger.get(addr2, 0), 1)  # Second block miner count increased
    ...
    def test_network_sync(self):
        ...
        # Wait for sync
        self.assertTrue(wait_for_chain_sync(beacons, 2))
    ...
    def test_randomness(self):
        ...
        coinbase = generate_eth_address()
        beacon = RandomnessBeacon(coinbase_address=coinbase, difficulty_bits=self.__DIFFICULTY)  # Use low difficulty for test
        ...
        print(f"Bit distribution: {zeros} zeros, {ones} ones ({zeros/(zeros+ones)*100:.2f}% zeros)")
    ...
    def test_randomness_distribution(self):
        ...
        for i, block in enumerate(beacon.blocks):
            ...
            if i < 5:  # Just show first few to avoid too much output
                print(f"Block {i}: original={hash_hex[:8]}... rehashed={rehashed[:8]}...")
                print(f"  Bits: {zeros} zeros, {ones} ones ({zeros/(zeros+ones)*100:.2f}% zeros)")
