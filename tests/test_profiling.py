    ...
    def test_message_creation_and_broadcast(self):
        ...
        with profile("message_creation_and_broadcast"):
            for i in range(num_messages):
                data = {"message_type": "TEST", "content": f"Test message {i}", "timestamp": time.time()}
        ...
        try:
            ...
            with profile("message_handling"):
                for i in range(num_messages):
                    ...
                    # Small delay to avoid overwhelming the network
                    time.sleep(0.0002)  # Adjusted delay to keep test duration reasonable
        ...
        try:
            with profile("db_operations"):
                ...
                for key in list(db_node.db.keys())[:20]:
                    if key != db_node.PEERS.encode() and key != db_node.BANNED_PEERS.encode():
                        _ = db_node.db[key]
