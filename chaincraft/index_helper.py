...
class IndexHelper:
    ...
    def initialize_database(self) -> None:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_message_hash ON indexed_messages(message_hash)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_field_value ON field_values(field_value)')
    ...
    def set_indexed_fields(self, message_type: str, fields: List[str]) -> None:
        ...
        cursor.execute('INSERT OR IGNORE INTO message_types (type_name) VALUES (?)', (message_type,))
        cursor.execute('SELECT id FROM message_types WHERE type_name = ?', (message_type,))
        type_id = cursor.fetchone()[0]
        
        # Add indexed fields
        for field in fields:
            cursor.execute('INSERT OR IGNORE INTO indexed_fields (type_id, field_name) VALUES (?, ?)', 
                          (type_id, field))
        ...
        try:
            ...
            for field in self.indexed_fields[message_type]:
                if field in message_data:
                    ...
                    # Get the field_id
                    cursor.execute('SELECT id FROM indexed_fields WHERE type_id = ? AND field_name = ?', 
                                  (type_id, field))
