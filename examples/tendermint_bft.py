...
class TendermintBFT(SharedObject):
    ...
    def is_valid(self, message: SharedMessage) -> bool:
        ...
        try:
            ...
            if message_type not in ["TENDERMINT_PROPOSAL", "TENDERMINT_PREVOTE", 
                                    "TENDERMINT_PRECOMMIT", "TENDERMINT_BLOCK"]:
                return False
                
            # Always print received Tendermint messages for debugging
            print(f"Received message of type {message_type} for height {data.get('height', 'unknown')}")
    ...
    def _validate_proposal(self, proposal: Dict[str, Any]) -> bool:
        ...
        required_fields = ["height", "round", "prev_block_hash", "proposer", "signature"]
        ...
        if proposal["height"] != self.current_height:
            print(f"Proposal height mismatch: got {proposal['height']}, current {self.current_height}")
            return False
    ...
    def _validate_vote(self, vote: Dict[str, Any]) -> bool:
        ...
        if vote["height"] != self.current_height:
            print(f"Vote height mismatch: got {vote['height']}, current {self.current_height}")
            return False
    ...
    def _validate_block(self, block: Dict[str, Any]) -> bool:
        ...
        # Check required fields
        required_fields = ["height", "round", "prev_block_hash", "proposer", "validators", "signatures"]
    ...
    def _process_consensus_message(self, message: SharedMessage) -> None:
        ...
        if message_type == "TENDERMINT_PROPOSAL":
            ...
            if self.current_step != ConsensusStep.PROPOSE:
                print(f"Received proposal while in {self.current_step.name}, moving to PROPOSE")
                self.current_step = ConsensusStep.PROPOSE
    ...
    def _process_proposal(self, proposal: Dict[str, Any]) -> None:
        """Process a proposal"""
        print(f"Node {self.validator_address[:10]}...: Processing proposal from {proposal.get('proposer', 'unknown')[:10]}... for height {proposal.get('height', 'unknown')}")
        
        # Validate the proposal height and round
        if proposal.get("height") != self.current_height or proposal.get("round") != self.current_round:
            print(f"Node {self.validator_address[:10]}...: Ignoring proposal for wrong height/round")
            return
        
        # Set the current proposal
        self.current_proposal = proposal
        print(f"Node {self.validator_address[:10]}...: Accepting proposal for height {self.current_height}")
        
        # If we're the proposer, we've already prevoted for our own proposal
        if self.current_step == ConsensusStep.PROPOSE and proposal.get("proposer") == self.validator_address:
            print(f"Node {self.validator_address[:10]}...: Moving to PREVOTE for my own proposal")
        ...
        if self.current_step == ConsensusStep.PROPOSE:
            print(f"Node {self.validator_address[:10]}...: Moving to PREVOTE for height {self.current_height}")
            self.current_step = ConsensusStep.PREVOTE
            ...
            print(f"Node {self.validator_address[:10]}...: Received proposal while in {self.current_step.name}, not changing state")
    ...
    def _process_prevote(self, prevote: Dict[str, Any]) -> None:
        ...
        if height < self.current_height:
            print(f"Node {self.validator_address[:10]}...: Ignoring outdated prevote for height {height}")
            return
        ...
        print(f"Node {self.validator_address[:10]}...: Added prevote from {validator[:10]}... for height {height}")
    ...
    def _process_precommit(self, precommit: Dict[str, Any]) -> None:
        ...
        print(f"Node {self.validator_address[:10]}...: Added precommit from {validator[:10]}... for height {height}")
    ...
    def _check_prevotes(self) -> None:
        ...
        proposal_hash = self._calculate_proposal_hash(self.current_proposal) if self.current_proposal else "nil"
        matching_prevotes = 0
        ...
        print(f"Node {self.validator_address[:10]}...: Prevote count: {matching_prevotes}/{len(self.validators)}, need {threshold}")
        
        if matching_prevotes >= threshold:
            print(f"Node {self.validator_address[:10]}...: ✓ Sufficient prevotes, moving to PRECOMMIT")
    ...
    def _check_precommits(self) -> None:
        ...
        print(f"Node {self.validator_address[:10]}...: Precommit count: {matching_precommits}/{len(self.validators)}, need {threshold}")
        ...
        if matching_precommits >= threshold:
            ...
            if proposal_hash != "nil" and self.current_proposal:
                print(f"Node {self.validator_address[:10]}...: ✓ Sufficient precommits, creating block")
                ...
                print(f"Node {self.validator_address[:10]}...: ✓ Sufficient precommits for nil proposal, moving to next round")
    ...
    def _has_quorum_votes(self, votes: Dict[str, Dict[str, Any]]) -> bool:
        ...
        if vote_counts:
            print(f"Vote counts: {vote_counts}, Required: {required_votes}, Validators: {len(self.validators)}")
        
        # Check if any block hash has enough votes
        for block_hash, count in vote_counts.items():
            if count >= required_votes:
                print(f"Quorum reached for block {block_hash[:8]}... with {count} votes")
                return True
    ...
    def _send_prevote(self, proposal: Dict[str, Any]) -> None:
        ...
        print(f"Sending prevote for block {block_hash[:8]}... at height {self.current_height}")
    ...
    def _create_block(self) -> None:
        ...
        if not self.current_proposal:
            print(f"Node {self.validator_address[:10]}...: Cannot create block without a proposal")
            return
        ...
        print(f"Node {self.validator_address[:10]}...: Created block at height {self.current_height}")
        
        # Notify other validators about the new block
        for validator in self.validators:
            if validator != self.validator_address:
                self._send_message(validator, {"type": "commit", "height": self.current_height})
    ...
    def _advance_height(self) -> None:
        ...
        if wait_time > 0:
            print(f"Node {self.validator_address[:10]}...: Waiting {wait_time:.2f}s before starting height {self.current_height + 1}")
        ...
        print(f"Node {self.validator_address[:10]}...: ➡️ Moving to height {self.current_height}")
        ...
        if self.current_height in self.pending_proposals:
            ...
            print(f"Node {self.validator_address[:10]}...: Processing pending proposal for height {self.current_height}")
    ...
    def start_consensus(self) -> None:
        ...
        with self.consensus_lock:
            ...
            if self._is_proposer():
                ...
                print(f"Waiting for proposal from {proposer[:10]}... for height {self.current_height}")
    ...
    def _is_proposer(self) -> bool:
        ...
        print(f"Height {self.current_height}: Checking if I ({self.validator_address[:10]}...) am the proposer.")
    ...
    def _send_proposal(self) -> None:
        ...
        if not self._is_proposer():
            print(f"Node {self.validator_address[:10]}...: Not the proposer for height {self.current_height}")
            return
            
        print(f"Node {self.validator_address[:10]}...: I AM the proposer for height {self.current_height}!")
        ...
        self.current_proposal = proposal
        print(f"Node {self.validator_address[:10]}...: Created proposal for height {self.current_height}")
    ...
    def _process_message(self, message):
        ...
        if message_type == "proposal" or message_type == "TENDERMINT_PROPOSAL":
            print(f"Node {self.validator_address[:10]}...: Received PROPOSAL from {message.get('proposer', 'unknown')[:10]}...")
            self._process_proposal(message)
            
        elif message_type == "prevote" or message_type == "TENDERMINT_PREVOTE":
            print(f"Node {self.validator_address[:10]}...: Received PREVOTE from {message.get('validator', 'unknown')[:10]}...")
            self._process_prevote(message)
            
        elif message_type == "precommit" or message_type == "TENDERMINT_PRECOMMIT":
            print(f"Node {self.validator_address[:10]}...: Received PRECOMMIT from {message.get('validator', 'unknown')[:10]}...")
            self._process_precommit(message)
            
        elif message_type == "commit" or message_type == "TENDERMINT_BLOCK":
            print(f"Node {self.validator_address[:10]}...: Received COMMIT notification")
            # Update our state if we receive a commit message for a new block
            if message.get("height") > self.current_height:
                self._advance_height()
                
        else:
            print(f"Node {self.validator_address[:10]}...: Received unknown message type: {message_type}")
    ...
    def _send_message(self, target_validator: str, message: Dict[str, Any]) -> None:
        ...
        for node in self.nodes:
            if hasattr(node, "validator_address") and node.validator_address == target_validator:
                node._process_message(message)
                return
                
        print(f"Warning: Could not find validator {target_validator[:10]}... to send message")

    def _broadcast_prevote(self) -> None:
        """Broadcast a prevote for the current proposal"""
        if not self.current_proposal:
            print(f"Node {self.validator_address[:10]}...: Cannot prevote without a proposal")
            return
        ...
        # Add our own prevote
        self.prevotes[self.current_height].append(prevote)
        print(f"Node {self.validator_address[:10]}...: Created PREVOTE for height {self.current_height}")
    ...
    def _broadcast_precommit(self) -> None:
        ...
        if not self.current_proposal:
            print(f"Node {self.validator_address[:10]}...: Cannot precommit without a proposal")
            return
        ...
        self.precommits[self.current_height].append(precommit)
        print(f"Node {self.validator_address[:10]}...: Created PRECOMMIT for height {self.current_height}")
    ...
    def _check_step_timeout(self):
        ...
        if timeout > 0 and elapsed > timeout:
            print(f"Node {self.validator_address[:10]}...: Timeout in {self.current_step.name} after {elapsed:.2f}s")
            ...
            if self.current_step == ConsensusStep.PROPOSE:
                ...
                if not self.current_proposal and self._is_proposer():
                    ...
                    print(f"Node {self.validator_address[:10]}...: No proposal received, sending nil vote")
                    self.current_step = ConsensusStep.PREVOTE
                ...
                self.current_step = ConsensusStep.PROPOSE
                self.current_proposal = None
                print(f"Node {self.validator_address[:10]}...: Moving to round {self.current_round}")
    ...
    def _broadcast_prevote_nil(self):
        ...
        # Add our own prevote
        self.prevotes[self.current_height].append(prevote)
        print(f"Node {self.validator_address[:10]}...: Created NIL PREVOTE for height {self.current_height}")
    ...
    def _broadcast_precommit_nil(self):
        ...
        # Add our own precommit
        self.precommits[self.current_height].append(precommit)
        print(f"Node {self.validator_address[:10]}...: Created NIL PRECOMMIT for height {self.current_height}")
    ...
    def _is_valid_proposal(self, proposal) -> bool:
        ...
        # Check required fields
        if "height" not in proposal or "round" not in proposal or "proposer" not in proposal:
            return False
...
class TendermintNode:
    ...
    def _run(self):
        ...
        while self.running:
            ...
            if self.consensus.current_step == ConsensusStep.PROPOSE and self.consensus._is_proposer():
...
def create_tendermint_network(num_nodes=3, consensus_interval=5):
    ...
    for i, node in enumerate(nodes):
        ...
        print(f"Node {i} initialized with validator address: {tendermint.validator_address[:10]}...")
        print(f"Known validators: {[addr[:10] + '...' for addr in tendermint.validators]}")
        ...
        while True:
            ...
            if all(h == current_heights[0] for h in current_heights):
                if current_heights[0] > 1:
                    print(f"✓ Successfully reached consensus at height {current_heights[0]}")
                    
            # Check if consensus has stalled completely
            if all(step == "PROPOSE" for step in current_steps) and no_progress_count >= 3:
                # Determine who should be the proposer and print debug info
                for validator in validators:
                    if validator.consensus._is_proposer():
                        print(f"Validator {validator.consensus.validator_address[:10]}... should be proposing but isn't")
