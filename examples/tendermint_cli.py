...
class TendermintCLI:
    ...
    def init_network(self, args) -> None:
        ...
        validator_info = {
            ...
            "host": config.get('public_host', socket.gethostbyname(socket.gethostname())),
            "port": config['port']
        }
        ...
        print(f"To start this node, run: python {sys.argv[0]} start --config {args.config}")
        print(f"For other nodes to join, share genesis.json and update their seed_nodes config.")
    ...
    def join_network(self, args) -> None:
        ...
        if not args.genesis:
            ...
            print("Usage: python tendermint_cli.py join --config CONFIG --genesis GENESIS")
            sys.exit(1)
        ...
        for validator in genesis['validators']:
            if validator not in config['validators']:
                ...
                seed_exists = False
                for seed in config['seed_nodes']:
                    if seed['host'] == validator['host'] and seed['port'] == validator['port']:
                        seed_exists = True
                        break
    ...
    def _display_status(self) -> None:
        ...
        print(f"Latest Block: {self.tendermint.blocks[-1]['hash'][:8]}... @ {self.tendermint.blocks[-1]['height']}")
    ...
    def add_validator(self, args) -> None:
        ...
        if not args.address or not is_valid_address(args.address):
            ...
            print("Usage: python tendermint_cli.py add-validator --address VALIDATOR_ADDRESS")
            return
    ...
    def run(self) -> None:
        ...
        # Init command
        init_parser = subparsers.add_parser("init", help="Initialize a new Tendermint network")
        init_parser.add_argument("--config", required=True, help="Path to configuration file")
        
        # Start command
        start_parser = subparsers.add_parser("start", help="Start a Tendermint node")
        start_parser.add_argument("--config", required=True, help="Path to configuration file")
        
        # Join command
        join_parser = subparsers.add_parser("join", help="Join an existing Tendermint network")
        join_parser.add_argument("--config", required=True, help="Path to configuration file")
        join_parser.add_argument("--genesis", required=True, help="Path to genesis file")
        ...
        validator_parser = subparsers.add_parser("add-validator", help="Add a new validator")
        validator_parser.add_argument("--address", required=True, help="Validator address")
        validator_parser.add_argument("--config", required=True, help="Path to configuration file")
